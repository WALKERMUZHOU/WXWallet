...
static NSInteger kCodeAmountLimitFloor      = 1023; // "金额低于最低限制"
static NSInteger kCodeAgeLimitFloor         = 1024; // "年龄低于最低限制"
static NSInteger kcodeFundationLimitFloor   = 1080;//资产页面

static NSInteger kCodeApplyingBorrowOrder   = 1028;   // "您有处理中的借款申请",已有借款申请
static NSInteger kCodeApplyingBorrowRepeat  = 1030;   // "借款申请已提交成功，请勿重复提交"
static NSInteger kCodeBorrowStepAlert       = 1000;   // 借款最后一步弹框code
static NSInteger kCodeCertificationOtherFailed = 1044;   // 三方认证失败，重新认证
static NSInteger kCodeCertificationFailed   = 1047;   // 认证失败，信息不一致，重新认证
static NSInteger kCodeCertificationGoingOn  = 1048;   // 认证正在进行中
static NSString * const kGetLoanIdentity    = @"/api/app/next/user/large/identity";     // 验证社保公积金
...
@interface  LWBaseCoordinatorRequestItem: NSObject
...
@class LWBaseCoordinator;
@protocol LWCoordinatorDelegate <NSObject>
@optional
...
- (void)coordinator:(LWBaseCoordinator *)coordinator requestEndAllCompleted:(BOOL)completed;//if completed is YES,load All data

- (void)coordinator:(LWBaseCoordinator *)coordinator resultCode:(NSInteger)resultCode resultMessage:(NSString *)resultMessage;
- (void)coordinator:(LWBaseCoordinator *)coordinator resultCode:(NSInteger)resultCode data:(id)data resultMessage:(NSString *)resultMessage;

@end

@interface LWBaseCoordinator : NSObject

@property (assign, nonatomic) NSInteger tag;
@property (weak, nonatomic) id<LWCoordinatorDelegate> delegate;
@property (strong, nonatomic) NSDictionary *extraInfomation;
@property (assign, nonatomic) BOOL showErrorMessage;//default is YES;
@property (weak, nonatomic) LWBaseCoordinatorRequestItem *currentRequestItem;
